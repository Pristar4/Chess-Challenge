...
public static class MenuUI {
    public static void DrawButtons(ChallengeController controller) {
        var buttonPos = UIHelper.Scale(new Vector2(300, 210));
        var buttonSize = UIHelper.Scale(new Vector2(300, 55));
        ...
        if (NextButtonInRow("Human vs MyBotV2", ref buttonPos, spacing, buttonSize)) {
            var whiteType = controller.HumanWasWhiteLastGame
        ...
        if (NextButtonInRow("BrutalBot vs EvilBot", ref buttonPos, spacing, buttonSize)) {
            controller.StartNewBotMatch(ChallengeController.PlayerType.BrutalBot,
                                        ChallengeController.PlayerType.EvilBot);
        }

        if (NextButtonInRow("MyBot vs BrutalBot", ref buttonPos, spacing, buttonSize)) {
            controller.StartNewBotMatch(ChallengeController.PlayerType.MyBot,
                                        ChallengeController.PlayerType.BrutalBot);
        }
