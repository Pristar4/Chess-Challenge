...
public class BrutalBot : IChessBot {
    private struct Entry {
        public ulong Key;
        public short Score, Depth;
        public Move Move;
    }
    private static Entry[] _transpositions = new Entry[16777216];

    private static readonly ulong[] Knights = {
                0x3234363636363432ul, 0x34383c3d3c3d3834ul, 0x363c3e3f3f3e3c36ul,
                0x363c3f40403f3d36ul,
            },
            Bishops = {
                0x3c3e3e3e3e3e3e3cul, 0x3e4040414042413eul, 0x3e4041414242403eul,
                0x3e4042424242403eul,
            },
            Rooks = {
                0x6465636363636364ul, 0x6466646464646464ul, 0x6466646464646464ul,
                0x6466646464646465ul,
            },
            Queens = {
                0xb0b2b2b3b4b2b2b0ul, 0xb2b4b4b4b4b5b4b2ul, 0xb2b4b5b5b5b5b5b2ul,
                0xb3b4b5b5b5b5b4b3ul,
            };
    private float _budgetCounter = 121, budget;
    private System.Collections.Generic.HashSet<Move> _killerMoves = new();
    private bool _outOfTime;
    private Board board;
    private ulong[][] Starts = {
                null,
                new[] {
                    0x141e161514151514ul, 0x141e161514131614ul, 0x141e181614121614ul,
                    0x141e1a1918141014ul,
                },
                Knights, Bishops, Rooks,
                Queens,
                new[] {
                    0x0004080a0c0e1414ul, 0x020406080a0c1416ul, 0x020406080a0c0f12ul,
                    0x02040406080c0f10ul,
                },
            },
            Ends = {
                null,
                new[] {
                    0x14241e1a18161614ul, 0x14241e1a18161614ul, 0x0e1114171614100aul,
                    0x14241e1a18161614ul,
                },
                Knights, Bishops, Rooks,
                Queens,
                new[] {
                    0x0c0f0e0d0c0b0a06ul, 0x0e100f0e0d0c0b0aul, 0x0e1114171614100aul,
                    0x0e1116191815100aul,
                },
            };
    private Timer timer;
    ...
    public Move Think(Board b, Timer t) {
        board = b;
        timer = t;
        budget = Min(0.033333333333333333333333f, 2.0f / --_budgetCounter) *
                 t.MillisecondsRemaining;
        _outOfTime = false;
    ...
    private int EvaluateBoard() {
        float ownOneMinusEndgameT = OneMinusEndgameT(false),
                otherOneMinusEndgameT = OneMinusEndgameT(true),
                score = 0.0f;

        foreach (var pl in board.GetAllPieceLists()) {
            score += 0b1000010 >> (int)pl.TypeOfPieceInList != 0
                    ? (pl.IsWhitePieceList ? ownOneMinusEndgameT : -otherOneMinusEndgameT) *
                      EvaluatePieceSquareTable(Starts, pl)
                      + (pl.IsWhitePieceList
                              ? 1.0f - ownOneMinusEndgameT
                              : otherOneMinusEndgameT - 1.0f) * EvaluatePieceSquareTable(Ends, pl)
                    : EvaluatePieceSquareTable(Starts, pl);
        }
    ...
    private ulong EvaluatePieceSquareTable(ulong[][] table, PieceList pl) {
        ulong value = 0;

        foreach (var p in pl) {
            var sq = p.Square;
            value += table[(int)p.PieceType][sq.File >= 4 ? 7 - sq.File : sq.File] <<
                    8 * (pl.IsWhitePieceList ? 7 - sq.Rank : sq.Rank) >> 56;
        }

        return 25 * value;
    }

    private float OneMinusEndgameT(bool white) {
    ...
    private (int, Move, bool) Search(int depthLeft, int checkExtensionsLeft, bool isCaptureOnly,
                                     int alpha = -32200, int beta = 32200) {
        ...
        var trans = _transpositions[key % 16777216];
        ...
        if (isCaptureOnly && (score = EvaluateBoard()) > bestScore &&
            beta < (alpha = Max(alpha, bestScore = score)))
        ...
        foreach (var lmove in legal) {
            prioritizedMoves[loopvar++] = (
                    (trans.Key == key && lmove == trans.Move ? 5000 :
                            _killerMoves.Contains(lmove) ? 500 : 0)
        ...
        foreach (var (_, move) in prioritizedMoves) {
            ...
            try {
                if (depthLeft >= 3 && ++loopvar >= 4 && !move.IsCapture) {
                    score = -Search(depthLeft - 2, checkExtensionsLeft, isCaptureOnly, -beta,
                                    -alpha).Item1;
                ...
                (score, _, canUse) = Search(depthLeft - 1, checkExtensionsLeft, isCaptureOnly,
                                            -beta, -alpha);
        ...
        if (!_outOfTime && !isCaptureOnly && canUseTranspositions && bestScore != 0)
            _transpositions[key % 16777216] = new Entry {
                Key = key, Depth = (short)(approximate ? -depthLeft : depthLeft),
                Score = (short)bestScore, Move = best,
            };
